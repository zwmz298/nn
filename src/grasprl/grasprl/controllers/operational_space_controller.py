...
import numpy as np
from controllers.joint_effort_controller import JntEffortCtrl
class OSC(JntEffortCtrl):
    def __init__(
        self,
        physics,
        joints,
        ...
        self._eef_id = self._physics.bind(eef_site, obj_type='site').element_id
        self._jnt_dof_ids = self._physics.bind(joints, obj_type='joint').dofadr
    ...
    def run(self, target):
        ...
        dq = self._physics.bind(self._joints, obj_type='joint').qvel
        # Get the end-effector position, orientation matrix, and twist (spatial velocity).
        ee_pos = self._physics.bind(self._eef_site, obj_type='site').xpos
        ee_quat = mat2quat(self._physics.bind(self._eef_site, obj_type='site').xmat.reshape(3, 3))
        ...
        u += self._physics.bind(self._joints, obj_type='joint').qfrc_bias
