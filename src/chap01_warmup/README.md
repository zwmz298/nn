...
本项目包含一系列 NumPy 库的基础练习题，旨在帮助学习者掌握数组操作、数学运算、矩阵操作以及简单的绘图功能。
...
|---|---|---|---|
| `--seed` | int | 42 | 随机数生成种子 |
| `--save-dir` | str | None | 图像保存目录（不指定则不保存） |
| `--no-show` | flag | - | 不弹出图像窗口（配合 save-dir 使用） |

### 2.3 输出文件

当使用 `--save-dir outputs` 参数时，会生成以下文件：

| 文件 | 说明 |
|---|---|
| `outputs/quadratic.png` | 二次函数图像 |
| `outputs/trigonometric.png` | 正余弦函数图像 |

## 3. 题目说明

按照 [python 文件](https://github.com/OpenHUTB/nn/blob/main/src/chap01_warmup/numpy_tutorial.py) 中的要求，利用 NumPy 实现对应的操作。
...
| 题号 | 知识点 | 核心技能 |
|---|---|---|
| 1 | 库导入 | `import numpy as np` |
| 2-3 | 数组创建 | `np.array()`、形状和元素访问 |
| 4 | 特殊矩阵 | `np.zeros()`、`np.ones()`、`np.eye()`、`np.random.rand()` |
| 5-10 | 索引与切片 | 基础索引、切片、高级索引 |
...
| 13 | 加法 | `+`、`np.add()` |
| 14 | 减法 | `-`、`np.subtract()` |
| 15 | 乘法 | `*`、`np.multiply()`、`np.dot()` |
| 16 | 除法 | `/`、`np.divide()` |
| 17 | 开方 | `np.sqrt()` |
| 18 | 点积 | `x.dot(y)`、`np.dot()` |
| 19 | 求和 | `np.sum()`（支持 axis 参数） |
| 20 | 求平均 | `np.mean()`（支持 axis 参数） |
| 21 | 转置 | `x.T` |
| 22 | 指数 | `np.exp()` |
| 23 | 最大值索引 | `np.argmax()` |
