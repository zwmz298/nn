...
- **实时统计仪表盘**：提供丰富的实时统计信息展示，包括时间戳、帧计数、车辆密度、累计计数、精度指标、速度统计等
...
- **实时统计仪表盘**：
  - 左上角主面板：时间戳、帧计数、车辆密度（颜色指示）、累计计数、分车型统计
  - 右上角信息面板：准确率、F1分数、平均速度、最大速度
  - 底部状态栏：实时帧率（颜色指示）、快捷键提示
  - 自适应布局：根据视频分辨率自动调整面板位置
...
- 置信度分布条形图（CARLA版本）
- **实时统计仪表盘**：
  - 左上角主面板：显示时间戳、帧计数、车辆密度（根据数量显示绿/黄/红颜色）、累计计数、分车型统计
  - 右上角信息面板：显示准确率、F1分数、平均速度、最大速度
  - 底部状态栏：显示实时帧率（根据帧率显示绿/黄/红颜色）和快捷键提示
- 处理后的视频保存到指定路径
...
1. **原始版本** (`original`) - 基础车辆计数功能，包含实时统计仪表盘
...
**📊 实时统计仪表盘**
- **左上角主面板**：时间戳、帧计数、车辆密度（根据数量显示绿/黄/红颜色）、累计计数、分车型统计（Car/Bike/Bus/Truck）
- **右上角信息面板**：准确率、F1分数、平均速度、最大速度
- **底部状态栏**：实时帧率（根据帧率显示绿/黄/红颜色）、快捷键提示
- **自适应布局**：根据视频分辨率自动调整面板位置，避免重叠
