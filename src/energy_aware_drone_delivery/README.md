...
复现实验时可优先调整电池容量、安全余量、载重能耗系数和风场强度。电池容量与安全余量主要影响是否需要中途充电，载重能耗系数会改变任务排序的收益，风场强度则会让逆风路径更容易被规避。调整参数后建议同时查看 `metrics.json` 和 `battery_trace.csv`，确认路线可行性与电量曲线是否一致。
