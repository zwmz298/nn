...
本项目利用卷积神经网络（Convolutional Neural Network, CNN）实现对 MNIST 手写数字数据集的分类识别任务。

MNIST 数据集中的图片为 0～9 的手写数字图像，模型需要根据输入图片判断其对应的数字类别。
...
MNIST 数据集包含 60,000 张训练图片和 10,000 张测试图片，每张图片大小为 28×28 像素，图像内容为灰度手写数字。

该数据集规模适中、结构清晰，常用于图像分类、模式识别和深度学习入门实验。由于数据量较小，MNIST 可以在普通笔记本电脑的 CPU 环境下完成训练和测试，适合作为卷积神经网络模型的基础实验数据集。

## 题目要求

本实验需要分别完成 TensorFlow 版本和 PyTorch 版本的卷积神经网络代码。
...
1. `self.conv1` 中 `nn.Conv2d()` 的相关参数；
2. `self.conv2` 中 `nn.Conv2d()` 的相关参数；
3. `x = x.view()` 中的张量展平参数。

两个版本训练完成后，模型在测试集上的分类准确率应达到 96% 以上。
