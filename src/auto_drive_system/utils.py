...
class HParamCallback(BaseCallback):
    def __init__(self, config):
        """
        在训练开始时保存超参数和指标，并将它们记录到 TensorBoard 中。
        """
        super().__init__()
        self.config = config
    ...
    def _on_training_start(self) -> None:
        ...
        for k, v in self.config.items():
            if isinstance(v, str) and v.isnumeric():
                ...
                hparam_dict[k] = v.__str__()
        # 通过引用其标签来定义将在 Tensorboard 的 `HPARAMS` 选项卡中显示的指标
        # Tensorboard 将在 `SCALARS` 选项卡中查找并显示指标
        metric_dict = {
            "rollout/ep_len_mean": 0,
            "train/value_loss": 0,
        }
...
def lr_schedule(initial_value: float, end_value: float, rate: float):
    """
    学习率调度：
        从 initial_value 到 end_value，以 10 为底进行指数衰减。

    :param initial_value: 初始学习率。
    :param rate: 指数衰减速率。高值意味着学习率在早期快速下降。
    :param end_value: 学习率的最终值。
    :return: 根据剩余进度计算当前学习率的调度函数。
    """
    ...
    def func(progress_remaining: float) -> float:
        ...
        :param progress_remaining: 介于 0 和 1 之间的浮点值，表示剩余进度。
        :return: 当前学习率。
        """
        if progress_remaining <= 0:
            return end_value
...
class HistoryWrapperObsDict(gym.Wrapper):
    ...
    字典观测的 History 包装器。
    :param env: 环境
    :param horizon: 需要保留在历史记录中的步数。
    """

    def __init__(self, env: gym.Env, horizon: int = 2, obs_key: str = 'vae_latent') -> object:
        self.obs_key = obs_key
        assert isinstance(env.observation_space.spaces[obs_key], gym.spaces.Box)
        print("使用 HistoryWrapperObsDict 包装环境。")
        wrapped_obs_space = env.observation_space.spaces[self.obs_key]
        wrapped_action_space = env.action_space
...
def parse_wrapper_class(wrapper_class_str: str):
    ...
    :param wrapper_class_str: (str) 需要解析的字符串。
    :return: (type) 包装器类及其参数。
    """
    wrap_class, wrap_params = wrapper_class_str.split("_", 1)
    wrap_params = wrap_params.split("_")
    wrap_params = [int(param) if param.isnumeric() else param for param in wrap_params]
