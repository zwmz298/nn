...
```text
src/rain_sensor_fusion/visualization_results/
```
...
![heavy_rain_dashboard](visualization_results/heavy_rain_dashboard.png)
...
![clear_dashboard](visualization_results/clear_dashboard.png)

晴天场景中，相机可见度和 LiDAR 点云质量更稳定，因此 Camera 与 LiDAR 权重更高，风险评分较低，系统只需要保持监控。

### 7.3 小雨场景对照

![light_rain_dashboard](visualization_results/light_rain_dashboard.png)

小雨场景下，相机权重开始下降，Radar 权重上升，但风险分数仍低于减速阈值，因此动作建议仍为 `monitor`。

### 7.4 雨雾混合场景

![foggy_storm_dashboard](visualization_results/foggy_storm_dashboard.png)

雨雾混合场景中，雾气进一步降低视觉可靠性，系统继续提高 Radar 权重，并给出 `slow_down` 建议。

### 7.5 多场景对比图

![scenario_comparison](visualization_results/scenario_comparison.png)
...
![rain_intensity_sweep](visualization_results/rain_intensity_sweep.png)
